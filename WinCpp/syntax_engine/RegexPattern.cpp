...
constexpr uint32_t kRegexMatchLimit = 1000000;
constexpr uint32_t kRegexDepthLimit = 1000;
}
...
bool RegexPattern::Match(const std::string& text) const
{
  ...
  pcre2_match_context* matchContext = pcre2_match_context_create(nullptr);
  if (!matchContext)
  {
